#将效果最好的ernie模型(ERNIE_all_avg)结果蒸馏到transformer和BiLSTM中
import torch
...
def train(method):
    ...
    for epoch in range(num_epochs):
        ...
        student_tqdm_iterator = tqdm(
            data_loader_train,
            dynamic_ncols=True,#动态显示进度条
            desc=f"Epoch {epoch + 1}/{num_epochs}",
        )
        teacher_iterator = iter(teacher_data_loader_train)
        for stu_data in student_tqdm_iterator: #学生数据
            try:
                tea_data = next(teacher_iterator) #老师数据
            except StopIteration:
                print("teacher data is over")
            # 选取对应批次数据的输入和标签
            #学生
            batch_x, batch_y = stu_data[0].to(device), stu_data[1].to(device)
            #老师
            input_ids = tea_data[0].to(device)
            attention_mask = tea_data[1].to(device)
            token_type_ids = tea_data[2].to(device)
            # label = tea_data[3].to(device)
            # print(label)
            # print(batch_y)
            # 模型预测
            with torch.no_grad():
                y_tea = teacher_model(input_ids, attention_mask, token_type_ids, method)
            y_hat = model(batch_x, method)
            optimizer.zero_grad()  # 梯度清零
            loss=hard_loss_function(y_hat,batch_y)
            # print(loss)
            ditillation_loss=soft_loss_function(F.log_softmax(y_hat,dim=1),F.softmax(y_tea/T,dim=1))
            ...
            total_loss.append(loss_all.item())

            student_tqdm_iterator.set_postfix(
                loss=sum(total_loss) / len(total_loss),
                acc=sum(total_true) / (batch_size * len(total_true)),
            )

        student_tqdm_iterator.close()
...
if __name__ == "__main__":
    ...
    teacher_model_folder = "./best_ernie"
    ...
    num_epochs = 40
    lr = 1e-3
    part = 2  # 1,2,3 三个部分,分别为Bi-LSTM，Transformer，预训练的Transformer完成文本分类
    ...
    if part == 1 or part == 2:
        ...
        data_loader_train = DataLoader(
            dataset=dataset.train, batch_size=batch_size, shuffle=False#为了和老师的一至，这里不shuffle
        )
    ...
    for i in range(times):
        ...
        #                 pickle.dump(dataset, f)
        #     data_loader_train = DataLoader(
        #         dataset=dataset.train, batch_size=batch_size, shuffle=False
        #     )
        #     data_loader_valid = DataLoader(
        #         dataset=dataset.valid, batch_size=batch_size, shuffle=False
        #     )
        ...
        if part == 1:
            print("bi-lstm")
            #加载老师网络
            teacher_model = torch.load(os.path.join(teacher_model_folder, "model.ckpt")).to(device)
            teacher_model.eval()
            if os.path.exists("./dataset_ernie.pkl"):
                    with open("./dataset_ernie.pkl", "rb") as f:
                        teacher_dataset = pickle.load(f)
            else:
                print("error at teacher_dataset")
            teacher_data_loader_train = DataLoader(
                dataset=teacher_dataset.train, batch_size=batch_size, shuffle=False
            )
            #加载学生网络
            model = BiLSTM_model(
            ...
            #学生网络读取先前训练好的参数
            # model.load_state_dict(torch.load("./best_lstm/model_state_dict.ckpt"))
        ...
        if part == 1 or part == 2:
            ...
            # 设置损失函数
            #硬损失
            hard_loss_function = nn.CrossEntropyLoss()
            #软损失
            soft_loss_function = nn.KLDivLoss()#reduction="mean") #reduction="batchmean"表示对batch求均值
            # 进行训练
            train(i)
